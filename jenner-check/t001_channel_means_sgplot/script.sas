/* Adapted from WholesaleCustomerSpendingAnalysis-Project.sas (Task 1) —
   original PROC IMPORT of the UCI wholesale-customers CSV replaced with an
   inline sample of the same 35-row shape so the bundle is self-contained.
   Data-step transformations, PROC SORT, PROC MEANS, and PROC SGPLOT are
