/* Adapted from WholesaleCustomerSpendingAnalysis-Project.sas (Task 3) —
   same inline sample as t001/t002, feeding PROC STEPDISC and PROC DISCRIM
   to test whether spending separates the three Region groups. Class/var
   lists and method options are unchanged from the source script; the
   `var fresh--delicassen;` positional range shorthand is spelled out as
   the equivalent explicit list (fresh milk grocery frozen
   detergents_paper delicassen) here. */
data wholesale;
...
proc discrim data=wholesale method=normal manova pool=test;
   class region;
   var fresh milk grocery frozen detergents_paper delicassen;
   priors proportional;
run;
